...
"""generate-from-table-v02.py reads in a .csv table with info on how to make lipids - creates all lipdis and can setup and run simulations, use generate-from-table-v02.py -h for description"""
...
# Options
#itpGenerator = "python3 ./lipid-itp-generator-Martini3-01-ILs_simplified.py " # calling the itp generator - options provided below 
itpGenerator = "python3 ./lipid-itp-generator-Martini3-01-ILs_simplified_ver2.py " # calling the itp generator - options provided below 
singleLipiditpFolder = "ils_itps"
...
def makeitps(tableData, options):
    ...
    for cline in tableData:
        ...
        if cData[0]=='-1':
            ...
            # -alink currently not used in the ionazable lipids generator
            current += "-alname " + cData[4] + " -alhead \"" + cData[20] + "\" -altail \"" + str(cData[22]).rstrip() + "\" -o " + itpFileName + " -name \""+ cData[9] +"\" "
...
def eqSim(sim):
    ...
    if use_CBT:
        ...
        subp.call(f'sed -ie "s/x_define_x/define                   = -DM3_NO_CBT/g" martini_v2.x_new-rf-prod.mdp', shell=True, cwd=csimDir)
